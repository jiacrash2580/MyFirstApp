...
public class MainActivity extends Activity
{
	...
	protected void onCreate(Bundle savedInstanceState)
	{
		...
		{
			e.printStackTrace();
			Toast.makeText(MainActivity.this, generateMsg(e.getLastError(), "Fail to unlock PDF library!!!", true), Toast.LENGTH_LONG).show();
			this.finish();
			return;
		}
		...
		catch (PDFException e)
		{
			if (e.getLastError() == PDFException.ERRCODE_PASSWORD)
			{
				...
				Toast.makeText(MainActivity.this, generateMsg(e.getLastError(), "Fail to open document!!!", true), Toast.LENGTH_LONG).show();
				bOpenSuccess = false;
				this.finish();			
				return;
			}
	...
	OnCheckedChangeListener toolbarListener = new OnCheckedChangeListener()	
	{
		...
		public void onCheckedChanged(RadioGroup group, int checkedId)
		{
			...
			case R.id.radio_annot:
			{
				...
				PopupMenu menu = new PopupMenu(MainActivity.this, display.getWidth() / 5, LayoutParams.WRAP_CONTENT);
			...
			case R.id.radio_rotate:
			{
				...
				break;
			case R.id.radio_otherfunctions:
			{
				WindowManager wm = (WindowManager)MainActivity.this.getApplicationContext().getSystemService(Context.WINDOW_SERVICE);
				Display display = wm.getDefaultDisplay();
				PopupMenu menu = new PopupMenu(MainActivity.this, display.getWidth() / 5, LayoutParams.WRAP_CONTENT);
	...
	OnItemOnClickListener annotItemOnClickListener = new OnItemOnClickListener()
	{
		...
		public void onItemClick(MenuItem item, int position)
		{
			...
			{
				//Toast or dialog
				Toast.makeText(MainActivity.this, "no annot license!!!", Toast.LENGTH_LONG).show();
				return;
			}
			switch (position)
			{
				...
				{	//click to start PSI operation
					if(pdfContext.getCurrentState().rotation == 0){
						if (viewController != null) {
							...
							{
								Toast.makeText(MainActivity.this, "no psi license!!!", Toast.LENGTH_LONG).show();
								return;
							}
